...
use mongodb::{
    bson::{
        doc,
        oid::{ObjectId as MongoObjectId, Error as OidError},
        Document
    },
...
impl ObjectId {
    ...
    pub fn new(hex_string: &str) -> Result<Self, OidError> {
        Ok(Self(MongoObjectId::with_string(hex_string)?))
    }
    // Creates a new instance of the scalar from a raw MongoDB ObjectId
    pub fn from_oid(oid: &MongoObjectId) -> Result<Self, OidError> {
        Ok(Self(oid.clone()))
    }
    ...
    pub fn find_clause_from_insertion_res(insertion_res: InsertOneResult) -> GQLResult<Document> {
        let oid_err = GQLError::new(String::from("Inserted object id couldn't be serialized as ObjectId"));

        let oid = ObjectId::from_oid(insertion_res.inserted_id.as_object_id().ok_or(
            oid_err // If this doesn't fail, nothing else will fail, and this really shouldn't fail (`_id` wouldn't be an ObjectId...)
        )?)?;
